    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                #获取人的坐标开始
                ######
                clss_t2n = clss.cpu().numpy() #检测类别转化为numpy
                person_index = np.argwhere(clss_t2n==0) #找到人的类别索引
                person_index = person_index.flatten() 
                print("person_index",person_index) 
                
                MyDevice = torch.device('cuda:0') # det_humans为人的坐标
                det_humans =  torch.rand(len(person_index), 4)
                det_humans = torch.tensor(det_humans, device=MyDevice) 
                for index,value in enumerate(person_index):
                    det_humans[index] = xywhs[value]
                print("det_humans",det_humans)
